...
namespace Simbir.Health.Account.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<DoctorInformationListDTO> GetAllDoctorsAsync(DoctorListFilterDTO filter)
        {
            var roles = await _context.Roles.Where(r => r.Name == "Doctor" || r.Name == "Admin")
                                            .ToListAsync();
            ...
            var userQuery =
                (
                    ...
                    join urr in _context.UserRoles on new { UserId = u.Id, RoleId = adminRole.Id } equals new { urr.UserId, urr.RoleId } into urrlj
                    from urr in urrlj.DefaultIfEmpty()
                    where !u.IsDeleted && ur.RoleId == doctorRole.Id && urr == null
            ...
            {
                userQuery = userQuery.Where(u => EF.Functions.Like(u.LastName.ToLower() + " " + u.FirstName.ToLower(), "%" + filter.NameFilter.ToLower() + "%"));
            }

            var totalCount = userQuery.Count();

            var userList = await userQuery.Skip(filter.From).Take(filter.Count).ToListAsync();

            var result = new DoctorInformationListDTO
            {
                Doctors = userList,
                TotalCount = totalCount
            };
        ...
        public async Task<DoctorInformationDTO> GetDoctorByIdAsync(int id)
        {
            var roles = await _context.Roles.Where(r => r.Name == "Doctor" || r.Name == "Admin")
                                            .ToListAsync();
            ...
            var doctor = await
                (
                    ...
                    join urr in _context.UserRoles on new { UserId = u.Id, RoleId = adminRole.Id } equals new { urr.UserId, urr.RoleId } into urrlj
                    from urr in urrlj.DefaultIfEmpty()
                    where u.Id == id && !u.IsDeleted && ur.RoleId == doctorRole.Id && urr == null
                    select new DoctorInformationDTO
                    {
                        Id = u.Id,
                        FirstName = u.FirstName,
